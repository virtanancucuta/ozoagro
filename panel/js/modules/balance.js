...
window.loadBalanceData = async function() {
  ...
  // CEDI: el canal 'mayorista' (venta de OZOAGRO a sus distribuidores) lo resuelve la misma RPC para el CEO
  ...
  {
    const canales = window.PERFIL && window.PERFIL.rol === 'ceo' ? ['web', 'tradicional', 'agente', 'mayorista'] : ['web', 'tradicional', 'agente'];
    let canalHtml = '';
      ...
      if (canalData && canalData[0]) {
        ...
        canalHtml += `
          <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
            <span class="font-medium ${c === 'mayorista' ? '' : 'capitalize'}">${c === 'mayorista' ? 'Mayorista (a distribuidores)' : c}</span>
