...
// 2026-09-21 · Video interactivo de distribuidor: click para reproducir, replay al terminar.
(function () {
  var v = document.getElementById('distribuidorVideoEl');
  var playBtn = document.getElementById('distribuidorPlayBtn');
  var replayBtn = document.getElementById('distribuidorReplayBtn');
  var container = v ? v.closest('.distributor-video-player') : null;
  if (!v || !playBtn || !container) return;

  function play() {
    v.play().then(function () {
      container.classList.add('playing');
      if (replayBtn) replayBtn.hidden = true;
    }).catch(function (e) { console.log('Video no pudo reproducirse:', e); });
  }
  ...
  // Carga el video solo al acercarse
  if ('IntersectionObserver' in window) {
    new IntersectionObserver(function (es, obs) {
      es.forEach(function (e) { if (e.isIntersecting) { v.load(); obs.disconnect(); } });
